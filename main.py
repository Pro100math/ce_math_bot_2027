...
# Глобальный массив — зашиваем задачи напрямую, чтобы база никогда не была пустой!
RAW_TASKS = [
    # --- ЧАСТЬ А ---
    ...
    (2023, 1, "В2", "В", "Чтение графиков", "В прямоугольном треугольнике гипотенуза равна 20, медиана равна 10. Чему равен радиус описанной окружности?", "10", "⚠️ Ловушка свойств! Радиус окружности, описанной около прямоугольного треугольника, всегда равен половине гипотенузы или длине медианы, проведенной к ней. R = 20 / 2 = 10."),
    (2023, 1, "В3", "В", "Геометрия на клетках", "Площадь ромба с диагоналями 12 и 16 увеличили в 3 раза. Найдите новую площадь.", "288", "⚠️ Ловушка формулы! Исходная S = (12 * 16) / 2 = 96. Новая площадь: 96 * 3 = 288."),
    ...
    (2023, 1, "В5", "В", "Тригонометрия", "Вычислите значение выражения: 20 · sin(150°) · cos(120°)", "-5", "⚠️ Ловушка знаков четвертей! sin(150°) = 0.5, cos(120°) = -0.5. Получаем: 20 * 0.5 * (-0.5) = -5."),
    (2023, 1, "В6", "В", "Тригонометрические выражения", "Найдите числовое значение выражения: 18√3 · tg(11π/3)", "-54", "⚠️ Ловушка периодов тангенса! tg(11π/3) = tg(-π/3) = -√3. Значение: 18√3 * (-√3) = -54."),
    (2023, 1, "В7", "В", "Алгебраические дроби", "Упростите выражение (x² - 9)/(x + 3) и найдите его значение при x = 15.", "12", "⚠️ Ловушка сокращения! (x-3)(x+3)/(x+3) = x - 3. При x = 15 получаем: 15 - 3 = 12."),
    ...
    (2023, 1, "В9", "В", "Иррациональные уравнения", "Решите уравнение: √(x + 1) = 3", "8", "⚠️ Возводим в квадрат: x + 1 = 9 => x = 8. Проверка ОДЗ пройдена."),
    (2023, 1, "В10", "В", "Показательные уравнения", "Найдите корень уравнения: 5^(x-3) = 125", "6", "⚠️ Приводим к одному основанию: 5^(x-3) = 5³ => x - 3 = 3 => x = 6.")
]
...
def init_db():
    ...
    # Автоматически проверяем и заливаем задачи прямо при старте!
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    builder.button(text="📚 Сборник 2023 г.", callback_data="year_2023")
    builder.adjust(1)
